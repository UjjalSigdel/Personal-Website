...
- `components/ui/` is a small shadcn/ui-style component set (Radix primitives + `class-variance-authority` + `tailwind-merge`), pruned in the Phase 1 cleanup to only the components actually rendered: `button`, `card`, `form`, `input`, `label`, `textarea`, `toast`/`toaster`, plus the site-specific `project-card`, `terminal-window` (the three-dot terminal chrome used by Hero/Skills/Contact), and `terminal-button` (a cva factory for the `$ command`-style buttons/links). Re-add other shadcn components (and their Radix deps) only when something actually uses them.
